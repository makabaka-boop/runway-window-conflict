...
2. **尚未入库拒绝**：入库时间 **严格晚于** 计算时刻的批次在该时刻尚未入库，
   属于不能动用的未来库存，整次请求在配给前返回 422（入库时间恰等于
   计算时刻视为已入库，可正常配给）；
3. **入库/失效时段合法**：每个批次的入库时间必须 **严格早于** 失效时间，
   倒挂或零长时段（入库等于失效）的批次没有合法库存时段，整次请求在
   配给前返回 422，错误定位到该批次的 `expires_at`；
4. **先到期先用**：有效批次按（失效时间、入库时间、批次编号）稳定排序后逐笔扣减，
   避免先到批次久置过期；**库存输入顺序不影响结果**；
5. **优先级扣减**：需求按请求中的顺序逐笔满足，每项需求可拆分到多个批次，
   拆分明细按扣减顺序列入 `lines`；
6. **三位小数精确计算**：数量以十进制 `Decimal` 精确运算，入参最多三位小数
   （`1.2300` 这类末尾带零的写法按数值本身判定，不超精度）；
7. **必要清单**：`batches` 与 `demands` 是两个必须显式给出的键，省略任一键
   都在对应字段返回 422（两个都省略时一次指出两个缺失清单）；显式提交
   空列表 `[]` 仍是合法的“无库存 / 无需求”配给；
8. **整次失败**：批次编号 / 作业编号任一重复、时间格式非法、数量非正（或超过
   三位小数、非有限、超出可表示范围）时整次请求返回 HTTP 422 字段级错误；
   **有效库存总量小于总需求**时同样整次 422，不返回任何部分配给，
   缺货反馈同时指出**总需求、有效库存与缺口**（过期与尚未入库批次均不计入有效库存）；
9. 响应中 `remaining` 覆盖全部有效批次（含被扣减至零的批次），
...
批次在计算时刻尚未入库（未来库存）时，在该批次的 `received_at` 处返回 422：

```json
{
  "detail": [
    {"type": "batch_not_received", "loc": ["body", "batches", 0, "received_at"],
     "msg": "批次 'FUTURE-1' 在计算时刻 2026-09-15T02:00:00Z 尚未入库（入库时间 2026-09-15T03:00:00Z），未来库存不得参与本次配给",
     "input": "2026-09-15T03:00:00Z",
     "ctx": {"batch_id": "FUTURE-1",
...
入库时间不严格早于失效时间（倒挂或零长时段）时，在该批次的 `expires_at`
处返回 422，与施工窗口倒挂区间使用同一错误类型：

```json
{
  "detail": [
    {"type": "start_not_before_end", "loc": ["body", "batches", 0, "expires_at"],
     "msg": "开始时间必须严格早于结束时间",
     "input": "2026-09-15T08:00:00Z"}
  ]
}
```

只提交计算时刻、省略 `batches` 与 `demands` 两个必要清单时，一次指出两个
缺失字段（显式提交空列表 `[]` 仍是合法配给）：

```json
{
  "detail": [
    {"type": "missing", "loc": ["body", "batches"], "msg": "Field required"},
    {"type": "missing", "loc": ["body", "demands"], "msg": "Field required"}
  ]
}
```
